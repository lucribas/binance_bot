...
class CandlestickPatterns
	...
	def initialize( param: )
		@cpc = CandlestickPatternsClassifier.new
		@param = param
	end
	...
	def pattern_classifier( candle:, position: )
		...
		sma_sum_close	= 0
		num	= 0
		for i in [1, position-@param[:SMA_PERIOD] ].max..position do
		...
		avg_bodysize = [avg_bodysize, @param[:BODY_SIZE]].min
		...
		sum_trade_qty = 0
		num	= 0
		for i in [1, position-@param[:VOL_AVG_PERIOD]].max..position do
			sum_trade_qty	= sum_trade_qty + candle[i][:trade_qty]
			num				= num + 1
		end
		avg_trade_qty = sum_trade_qty/num if num>0
		avg_trade_qty = [avg_trade_qty, @param[:VOL_SIZE]].min
		...
		c[:pattern] = @cpc.pattern( candle, position )
	end
	#+------------------------------------------------------------------+
end
